...
//		for (int s = 0; s < 1; s++) {
		for (int s = 0; s <= CLOSED_SITENUM; s++) {
			if (s < CLOSED_SITENUM) inum = CLOSED_INSTNUM;
			else inum = OPEN_INSTNUM;
			for (int i = 0; i < inum; i++) {
				for (int s2 = s; s2 <= CLOSED_SITENUM; s2++) {
					if (s2 < CLOSED_SITENUM) inum2 = CLOSED_INSTNUM;
					else inum2 = OPEN_INSTNUM;
		...
		fname = d["OUTPUT_LOC"] + d["LEV_METHOD"] + ".lev";
		...
		for (talking_core = 1; talking_core < CORE_TOTAL; talking_core++) {
			if ((int)joblist.size() > 0) {
				int my_job[4];
				copy(joblist[0].begin(), joblist[0].end(), my_job);
		...
		string folder = string(d["INPUT_LOC"]);
